...
public interface BandApplyInfoRepository extends JpaRepository<BandApplyInfoEntity, Long> {
    ...
    @Query("select bae from BandApplyInfoEntity bae left join fetch bae.user u where bae.band.id = :bandId")
    List<BandApplyInfoEntity> findUserByBandId(long bandId);
